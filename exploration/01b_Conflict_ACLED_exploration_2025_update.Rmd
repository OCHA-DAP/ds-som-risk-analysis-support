...
```{r}
date_run <- as.Date("2024-10-21")
yday(date_run)
df_acled_violence_gte2022 <- df_acled_violent %>%
  filter(
    year >= 2022,
         yday(date)<=yday(date_run)
         )

df_acled_violence_gte2022 |> glimpse()

df_adm1_acled_gte2022 <- df_acled_violence_gte2022 %>%
  group_by(year, admin1) %>%
  summarise(
    across(c("interaction", "fatalities"), ~ sum(as.numeric(.x)))
  ) 
...
## Increase/Decrease from 2024 to 2023
...
df_adm1_acled_compare %>%
  ...
  labs(
    ...
    caption = "All data filtered to same time-span (Jan 1- 21 October) as 2024 ACLED data to improve comparability"
  ) +
  theme(
    legend.title = element_blank(),
    axis.title.y = element_blank(),
...
ggplot() +
  ...
  labs(
    title = "Somalia - Increase/Decrease in conflict-related fatalities 2023-2024",
    subtitle = "Black lines delineate areas used in the 2023 HNO to define Al-Shabaab related conflict zones",
