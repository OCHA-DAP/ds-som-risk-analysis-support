...
gghdx()
buffer_som <-  c(T,F)[1] # this is just really for the viz files shared later
som_adm0_levels <- c(
...
df_ocha_pin <- readxl::read_xlsx(
  path = file.path(dir_ocha_explore,
                   "Workspace_3A-3B_PIN&Sev_2023-10-24_district level.xlsx"),
...
som_bbox <- st_bbox(som_adm$adm0) %>% 
  st_as_sfc()
plot(som_bbox)
plot(som_adm$adm0$geometry,add= T)


tnc_fs <- tidync(fp_fs)
tnc_fs_filt <- fs_filter_bounds(fs_obj = tnc_fs,
                                geometry = som_adm$adm0
                                )
if(buffer_som){
  tnc_fs_filt <- fs_filter_bounds(fs_obj = tnc_fs,
                                geometry = st_buffer(som_adm$adm0,10000)
                                )
}
...
4. Can we create a flood frac baseline?

The seasonal flood rasters produced here can be reclassified an polygonized as well (to share w/ SOM OCHA team)
```{r}
...
```{r}
r_fs_flood_frac_mam_avg <- mean(r_fs_seas_mam_max)
plot(r_fs_flood_frac_mam_avg)
plot(som_adm$adm0$geometry,add= T, fill=NULL)

r_fs_flood_frac_mam_avg_clip <- terra::mask(
  x = r_fs_flood_frac_mam_avg,
  st_buffer(som_adm$adm0,10000)
  )

# same as I did on initial slide visualization
r_fs_flood_frac_mam_avg_clip[r_fs_flood_frac_mam_avg_clip<=0.02] <- NA
plot(r_fs_flood_frac_mam_avg_clip)
plot(st_buffer(som_adm$adm0,10000)$geometry,add= T, fill=NULL)

poly_fs_flood_frac_mam_avg <- terra::as.polygons(r_fs_flood_frac_mam_avg_clip)
plot(poly_fs_flood_frac_mam_avg,add=T)
plot(r_fs_flood_frac_mam_avg_clip, add=T)

dir.create("gis_files_share")
poly_fs_flood_frac_mam_sf <- poly_fs_flood_frac_mam_avg %>% 
  st_as_sf() %>% 
  summarise()
plot(r_fs_flood_frac_mam_avg_clip)
plot(poly_fs_flood_frac_mam_sf$geometry,add=T)
plot(som_adm$adm0$geometry,add=T, fill=NULL)
st_write(poly_fs_flood_frac_mam_sf,"gis_files_share/SOM_MAM_1998_2016_FloodScan_SFED_baseline_extent.shp")


r_fs_flood_frac_mam_avg_clip %>% 
    values() %>%  
  data.frame() %>% 
...
df_adm1_pt_estimates_map <- df_adm2_both_seas_thresh_pop_adj %>% 
  group_by(
...
df_pin_adm1 <- df_pin_adm2 %>% 
  group_by(adm1_en, adm1_pcode) %>% 
  summarise(
    final_pin = sum(final_pin,na.rm=T),.groups = "drop"
  )
  
df_q_tbl_summary<- reduce(ldf_q_tbl,left_join)  %>% 
  left_join(df_pin_adm1)
...
## Quantile Ranges Admin 2 - MAM & OND
```{r}
df_adm1_flood_quantiles_both_seas <- df_adm1_historical_both_seas_pop_adj %>% 
  mutate(mo= month(date)) %>% 
  group_by(across(starts_with("adm")),mo) %>% 
...
df_adm2_flood_quantiles_both_seas <- df_adm2_historical_both_seas_pop_adj %>% 
  mutate(mo= month(date)) %>% 
  group_by(across(starts_with("adm")),mo) %>% 
    reframe(
    quibble(pct_pop_exposed_24,q=c(seq(0,0.95,0.05),0.99)),
...
ldf_adm2_q_tbl <- df_adm2_flood_quantiles_both_seas %>% 
  filter(mo==10 & pct_pop_exposed_24_q %in% c(0.25,0.75)|
         mo== 3 & pct_pop_exposed_24_q %in% c(0.5,0.95),
         ) %>% 
  rename(
...
df_adm2_q_tbl_summary<- reduce(ldf_adm2_q_tbl,left_join)  %>% 
  left_join(df_pin_adm2) %>% 
  select(
    adm1_en,adm2_en, everything(), - ends_with("_pcode")
  )

df_adm2_q_tbl_summary %>% 
  group_by(adm1_en) %>% 
  gt() %>% 
  gt::cols_label(
    adm1_en= "Region",
...
df_adm2_max_mam_ond_values <- df_adm2_historical_both_seas_pop_adj %>% 
...
adm2_map_figs_updated <- df_adm2_max_mam_ond_values %>% 
  left_join(
    df_pin_adm2 
      
  ) %>% 
  mutate(
    pct_pin = final_pin * pct_district_exposed
  )
...
adm2_map %>% 
  left_join(adm2_map_figs_updated) %>% 
  st_write(
    "som_flood.gpkg",
    layer = "adm2_flood_pop_mam_ond",
    delete_layer = T
