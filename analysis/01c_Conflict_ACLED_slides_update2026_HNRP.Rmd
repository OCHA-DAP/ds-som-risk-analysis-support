...
# cumulus::
gdf_adm1 <- cumulus::download_fieldmaps_sf(iso3= "som",layer = "som_adm1")$som_adm1
gdf_adm1 <- gdf_adm1 |> 
  clean_names()


api_base_url <- "https://acleddata.com/api/acled/read?_format=json"
# Updated ACLED API call using OAuth authentication (new system)
token_url <- "https://acleddata.com/oauth/token"
# Get credentials from environment variables
username <- Sys.getenv("ACLED_USER_EMAIL")
password <- Sys.getenv("ACLED_PASSWORD")

# Download ACLED data using new OAuth authentication
df_acled_long_new <- httr2::request(api_base_url) |>
  httr2::req_url_query(
    country = "Somalia",
    event_date = paste0("2010-01-01|", Sys.Date()),
    event_date_where = "BETWEEN",
    limit = 0  # No limit - get all records
  ) |>
  ...
  purrr::pluck("data") |>
  purrr::map(\(x) {
    # Remove NULL columns before converting to tibble
    x[!sapply(x, is.null)] |>
      dplyr::as_tibble()
  }) |>
  purrr::list_rbind() |>
...
# Filter to just violent events (same as original code)
df_acled_violent <- df_acled_long_new |> 
  filter(event_type %in% c(
    "Battles",
    "Explosions/Remote violence",
    "Violence against civilians"
...
# latest_date_used <- as_date("2023-10-13") # used for 2024 HNRP support (done in 2023)
# latest_date_used <- as_date("2024-11-28") # update
latest_date_used <- as_date("2025-11-19") # update
...
df_adm1_conflict_gte2022 %>%
  ...
  labs(
    ...
    subtitle = "Fatalities per admin 1 2022 to 2025",
...
df_adm1_conflict_gte2022 |> 
  ...
  ggplot(
    aes(x= year, y= fatalities, 
        color = conflict_class_label, group = conflict_class_label)
  )+
  geom_point(alpha = 0.3)+
  geom_line()+
  facet_wrap(~admin1) +
  labs(
    title = "Somalia: Yearly Conflict Fatalities",
    subtitle = "By admin 1 and conflict type classification (2022-2025)",
    caption = "Fatality Data: ACLED (Accessed 2025-11-19).\nYearly totals summed from 01 Jan to 19 Nov each year"
  )+
  theme(
    plot.caption = element_text(hjust=0),
    ...
    strip.background = element_rect(fill = "#1bb580"),
...
p_bar_yearly_diff <- df_adm1_conflict_acled_compare %>%
  ggplot(
  ...
  labs(
    title = "Somalia ACLED Data",
    subtitle = "Increase/Decrease Fatalities 2023-2024",
...
df_adm1_shabaab <- df_adm1_conflict_acled_compare %>% 
              filter(
                conflict_class_label =="Al Shabaab Related"
              )
gdf_adm1_acled_shabaab <- gdf_adm1_class %>%
  left_join(df_adm1_shabaab, by = c("adm1_en" = "admin1"))
...
p_shabaab_trends$Jubaland+
  ...
  plot_annotation(
    title = "Somalia: Yearly Al-Shabaab Related Conflict Fatalities",
    subtitle = "Split by 2023 HNO Regional Groupings"
    )+
  plot_layout(ncol=3)
```
...
```{r}
df_adm1_clan <- df_adm1_conflict_acled_compare %>% 
              filter(
                conflict_class_label =="Clan Related"
              )

gdf_adm1_acled_clan <- gdf_adm1_class %>%
  left_join(df_adm1_clan, by = c("adm1_en" = "admin1"))  
...
p_shabaab_map+
  labs(
    subtitle = "Al Shabaab conflict related fatalities"
  )+
  theme(
    plot.title= element_blank(),
    plot.subtitle = element_text(size =9),
    plot.caption = element_blank(),
    legend.position = c(0.95, 0.05),  # (x, y) coordinates in [0, 1] range
    legend.justification = c("right", "bottom"),  # Aligns to bottom-right corner
...
# legend.box.background = element_rect(color = NA, fill =NA)
  ...
  p_interclan_map+
      labs(
    subtitle = "Interclan-conflict related fatalities"
  ) +
  theme(
    plot.title= element_blank(),
    plot.caption = element_blank(),
  ...
  p_bar_yearly_diff+
  labs(
    subtitle = "2025 vs 2024 Fatalities"
  )+
  theme(
    plot.title= element_blank(),
    plot.subtitle = element_text(size =9),
  ...
  patchwork::plot_annotation(
    title= "Change in conflict related fatalities between 2025 & 2024", 
    caption = "ACLED data accessed 2025-11-19
Black lines delineate areas used in 2023 HNO to define Al-Shabaab vs Interclan violence zones. Grey fill on maps means no fatalities of respective classification in either 2025 or 2024.
Both years normalized to same date range (up to Nov. 19)",
    theme = theme(
      plot.caption = element_text(hjust = 0 )
    )
    )
