...
"../data/Somalia health indicators data for Jan-Sept 2024.xlsx"

fp_2024 <- file.path(
  "data",
  "Somalia health indicators data for Jan-Sept 2024.xlsx"
)
sn <- readxl$excel_sheets(fp_2024)




df_health <- map(sn,~readxl$read_excel(fp_2024,sheet = .x) |>
      mutate(
        date = dmy(paste0 ("01 ",str_sub(.x, start = 1, end = 3)," 2024")),.after = "District"
        )
      ) |> 
  list_rbind() |> 
  clean_names()
...
df_health |> 
  ggplot(
    aes(x= date ,y=acute_diarhoea_5, color = district)
  )+
  geom_line()
